This modules makes it easy to extract terraform state at terraform apply time and automatically update helm values files, kustomize 
files or other config files related to kubernetes clusters. This
module is useful when you don't wish to use the kubernetes or 
helm providers. 

Using it is very simple, and best understood by looking at the
examples. Say you have YOUR_TF_MODULE/main.tf, and you need to 
...
- in YOUR_TF_MODULE/main.tf, write something like this: 
  ...
  module "gen_helm_values" {
    ...
    deployment_roots = "/path/to/YOUR_APP_DIR/helm_values"
...
This will load the `$deployment_roots/_templates_/$tpl_name` file as 
...
to `$deployment_roots/stacks/$cluster_name/auto-root-$tpl_name`. The file
`$deployment_roots/_templates_/$tpl_name` could eg have the following: 
```
...
the file `$deployment_roots/stacks/auto-root-values.yaml` will be 
...
- you could specify a kubernetes namespace via the `k8s_ns` variable, 
  ...
  such as 
  `$deployment_roots/_templates_/$k8s_ns/$tpl_name`
  or even for a specific stack specific namespace in 
  `$deployment_roots/_templates_/$stack_id/$k8s_ns/$tpl_name`
  In that case the file
  `$deployment_roots/stacks/$stack_id/$k8s_ns/auto-values.yaml`
  will get regenerated whenever the cluster changes for that stack. 
  Then you could run 
  ```
  helm upgrade --install -f $deployment_roots/stacks/$stack_id/auto-root-values.yaml \
     -f $deployment_roots/stacks/$stack_id/$k8s_ns/auto-values.yaml
  ```
- you could have different modules used by your root terraform
  ...
  module2, you could have `$deployment_roots/_templates_/module1.yaml`
  and `$deployment_roots/_templates_/module2.yaml` and module1 and 2
